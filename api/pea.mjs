...
var _client = null;
var DEFAULT_URL = "https://nnsxyuhiwgrqbszrhyiz.supabase.co";
var DEFAULT_KEY = "sb_publishable_qMqdfCN5EcUiBh1zNcBVBQ_G-qmeQx5";
function getSupabase() {
  if (!_client) {
    const url = envGet("SUPABASE_URL") || DEFAULT_URL;
    const key = envGet("SUPABASE_SERVICE_ROLE_KEY") || envGet("SUPABASE_ANON_KEY") || DEFAULT_KEY;
...
function hashPassword(password) {
  ...
  const digest = sha2562(utf8Bytes(salt + String(password)));
...
var LOCALHOST_ONLY_MSG = "\u0E43\u0E0A\u0E49\u0E44\u0E14\u0E49\u0E40\u0E09\u0E1E\u0E32\u0E30 Localhost";
function truncateRemarks(val) {
  const s = val == null ? "" : String(val);
  return s.length > 2e3 ? s.substring(0, 2e3) : s;
}
function normalizeStatusPeriodKey(projectId, period) {
...
function getActiveUserEmail(fallbackUser) {
  return fallbackUser || "SYSTEM";
}
async function logAction(action, detail, user) {
  try {
...
async function logAuditRecord(userName, entityType, entityId, action, detail) {
  try {
    const supabase = getSupabase();
...
async function getSheetDataAsObjects(sheetName) {
  const supabase = getSupabase();
  const { data } = await supabase.from(sheetName).select("*");
  return data && Array.isArray(data) ? data : [];
}
async function getSheetDataAsObjectsTail(sheetName, maxRows = 100) {
  const limit = Math.max(1, maxRows);
  const supabase = getSupabase();
  const { data } = await supabase.from(sheetName).select("*").order("id", { ascending: false }).limit(limit);
  return data && Array.isArray(data) ? data : [];
}
...
function userDeptList(user) {
  if (!user) return [];
  return String(user.depts || "").split(",").map((d) => d.trim().toLowerCase()).filter(Boolean);
}
function userCanViewDeptVault(user, department, isPublic) {
  ...
  if (role === "viewer") {
    const depts2 = userDeptList(user);
    if (depts2.indexOf("all") >= 0) return true;
    return depts2.indexOf(String(department || "").trim().toLowerCase()) >= 0;
  }
  if (role !== "editor") return false;
  const depts = userDeptList(user);
  if (depts.indexOf("all") >= 0) return true;
  return depts.indexOf(String(department || "").trim().toLowerCase()) >= 0;
}
function userCanEditDeptVault(user, department) {
  if (!user) return false;
  const role = normalizeUserRole(user.role);
...
function userCanEditProjectDept(user, department) {
...
function userCanEditMaintWorkflow(user, dept) {
  return userCanEditDeptVault(user, dept);
}
function deptVaultFormatDate(val) {
  if (!val) return "";
  if (val instanceof Date) return val.toISOString();
  return String(val);
}
function deptVaultSanitizeForClient(item) {
  return {
    ID: item.ID || "",
...
function normalizeActualDateKey(val) {
  ...
  const parts = s.split(/[\s\-\/]+/).map((p) => parseInt(p, 10)).filter((n) => !isNaN(n));
  if (parts.length >= 3) {
...
function computeLicenseDurationYears(startDate, expiryDate) {
  ...
  const years = diffMs / (365.25 * 24 * 60 * 60 * 1e3);
...
async function getUserRecord(userName) {
  ...
  const supabase = getSupabase();
  const { data } = await supabase.from("Users").select("Username, Role, Depts").eq("Username", String(userName).trim());
...
async function ensureDefaultLicenseTypes() {
  const supabase = getSupabase();
...
async function ensureSettingsDefaults() {
  ...
  const existing = {};
  rows.forEach((r) => {
    if (r.Key) existing[String(r.Key)] = true;
  });
  const supabase = getSupabase();
...
var SESSIONS_KEY = "USER_SESSIONS";
...
async function restoreSession(username, token) {
  try {
...
function cachePut(key, data, ttlSeconds = CACHE_TTL_SECONDS) {
  cacheStore.set(key, { data, expiresAt: Date.now() + ttlSeconds * 1e3 });
}
...
function buildStatusesMap(rows, statusFrom, statusTo, filterStatuses) {
  const statuses = {};
  rows.forEach((r) => {
...
function buildCycleOverridesMap(rows) {
...
function buildCompaniesList(projects) {
  const companies = [];
  const compSet = {};
  projects.forEach((p) => {
...
function joinMaintWorkflows(workflows, steps) {
  const stepsByWf = {};
  steps.forEach((s) => {
...
async function loadMaintWorkflows() {
...
async function batchFetchTables(tableQueries) {
  const supabase = getSupabase();
  const results = {};
  await Promise.all(
...
async function fetchAllDataParallel(statusFrom, statusTo, filterStatuses, coreOnly) {
  const tables = [
  ...
  if (batch.projects == null) return { projects: [] };
  ...
  const depts = (batch.deptsRaw || []).map((r) => ({ name: r.Name || "", link: r.FolderLink || "" })).filter((d) => d.name);
...
async function fetchAllDataSequential(statusFrom, statusTo, filterStatuses, coreOnly) {
  ...
  const depts = (await getSheetDataAsObjects("Departments")).map((r) => ({ name: r.Name || "", link: r.FolderLink || "" })).filter((d) => d.name);
...
async function getLogsData() {
  try {
    const cached = cacheGet("ALL_DATA_LOGS_V3");
    if (cached) {
...
async function getAllData(options = {}) {
  try {
    ...
    if (!filterStatuses) {
      ...
      if (cached && cached.status === "success" && (cached.projects || []).length > 0) {
        if (coreOnly) return cached;
        const logsCached = cacheGet("ALL_DATA_LOGS_V3");
        if (logsCached) {
    ...
    if (!filterStatuses && result.projects.length > 0) {
      const corePayload = {
        status: "success",
      ...
      if (!coreOnly && (result.logs.length || result.auditLogs.length)) {
...
async function getDeptVaultRowsCached() {
...
async function authenticateUser(username, password) {
  try {
    ...
    for (const r of data) {
      if (String(r.Username).trim().toLowerCase() === uIn && verifyPassword(String(password), r.Password)) {
        if (!isPasswordHashed(r.Password)) {
          await supabase.from("Users").update({ Password: hashPassword(String(password)) }).eq("Username", r.Username);
          invalidateAllDataCache();
        }
        await logAction("LOGIN", "\u0E40\u0E02\u0E49\u0E32\u0E2A\u0E39\u0E48\u0E23\u0E30\u0E1A\u0E1A\u0E2A\u0E33\u0E40\u0E23\u0E47\u0E08", String(r.Username));
        const token = await createSession(r.Username);
        return {
          status: "success",
          username: r.Username,
          role: normalizeUserRole(r.Role),
...
async function getAdminUsersData(_requestingUser) {
  try {
    const supabase = getSupabase();
...
async function adminSaveUser(uData, requestingUser) {
  try {
...
async function adminDeleteUser(username, requestingUser) {
  try {
...
async function updateSystemSettings(bannerText, bannerActive, userName, phaseLabel, sessionTimeoutMinutes) {
  try {
    let safeText = bannerText == null ? "" : String(bannerText);
    if (safeText.length > 2e3) safeText = safeText.substring(0, 2e3);
    ...
    let safeTimeout = sessionTimeoutMinutes != null ? String(parseInt(String(sessionTimeoutMinutes), 10) || 480) : "";
...
async function saveProjectBulk(projectsArray) {
  try {
    ...
    for (const proj of projectsArray) {
      ...
      const payload = {
        ...
        Remarks: truncateRemarks(proj.Remarks !== void 0 ? proj.Remarks : proj.remarks)
      };
    ...
    await logAction(
      ...
      projectsArray[0] && projectsArray[0].userName || "System"
    );
    return { status: "success" };
  } catch (error) {
...
async function deleteProject(id, userName) {
  try {
...
async function updateFullProject(subIds, data) {
  try {
    ...
    for (const id of idsToUpdate) {
      ...
      payload.ID = id;
      payload.id = id;
      if (payload.Remarks === void 0 && payload.remarks !== void 0) {
...
async function updateProjectsPhaseBulk(targetPhase, checkedIds, uncheckedIds, userName) {
  try {
...
async function updateBulkStatus(updates, userName) {
  try {
    ...
    for (const uData of updates) {
      ...
      const newStat = uData.status !== void 0 ? uData.status : "\u0E23\u0E2D\u0E19\u0E33\u0E2A\u0E48\u0E07";
      const newInsp = uData.inspector !== void 0 ? String(uData.inspector) : "";
      const newExec = uData.execChecked !== void 0 ? uData.execChecked : false;
...
async function updateCycleOffset(projectId, offset, userName) {
  try {
...
async function updateMaintenanceVisit(data) {
  try {
...
async function updateMaintenanceVisitCore(data, dupMsg) {
  ...
  const targetIds = data.targetProjectIds && data.targetProjectIds.length ? data.targetProjectIds.map((id) => String(id || "").trim()).filter(Boolean) : [String(data.projectId || "").trim()].filter(Boolean);
  const supabase = getSupabase();
  if (!rowId) {
    ...
    if (fallbackPid && fallbackVno) {
      const { data: findRes } = await supabase.from("Maintenance").select("RowId").eq("ProjectID", fallbackPid).eq("VisitNo", fallbackVno);
  ...
  if (!rowId) {
    ...
    for (const pid of targetIds) {
      const { data: dupCheck } = await supabase.from("Maintenance").select("RowId").eq("ProjectID", pid).eq("ActualDate", data.actualDate);
    ...
    if (!existing || existing.length === 0) {
      ...
      for (const pid of targetIds) {
        const { data: edupCheck } = await supabase.from("Maintenance").select("RowId").eq("ProjectID", pid).eq("ActualDate", data.actualDate).neq("RowId", rowId);
      ...
      const { data: dupCheck2 } = await supabase.from("Maintenance").select("RowId").eq("ProjectID", editProjectId).eq("ActualDate", data.actualDate).neq("RowId", rowId);
...
async function deleteMaintenanceVisit(rowId, pId, visitNo, userName) {
  try {
...
async function findActiveMaintWorkflow(groupKey, visitNo) {
  const supabase = getSupabase();
  const { data } = await supabase.from("MaintWorkflow").select("WorkflowID").eq("GroupKey", String(groupKey)).eq("VisitNo", String(visitNo)).eq("Status", "in_progress");
  return data && data.length > 0 ? { WorkflowID: data[0].WorkflowID } : null;
}
function stepHasDataInRows(rows, workflowId, stepNo) {
...
async function maintWorkflowStepHasData(workflowId, stepNo) {
  const steps = await getSheetDataAsObjects("MaintWorkflowSteps");
  return stepHasDataInRows(steps, workflowId, stepNo);
}
async function upsertMaintWorkflowStepWithData(workflowId, stepNo, letterNo, stepDate, note, userName) {
  const supabase = getSupabase();
  const now = (/* @__PURE__ */ new Date()).toISOString();
...
async function autoSkipMaintWorkflowSteps(workflowId, targetStep, userName) {
  ...
  const supabase = getSupabase();
  const { data: wfRes } = await supabase.from("MaintWorkflow").select("CurrentStep").eq("WorkflowID", String(workflowId));
...
async function startMaintWorkflow(data, userName) {
  try {
    const user = await getUserRecord(userName);
    ...
    await supabase.from("MaintWorkflow").insert({
      ...
      TargetProjectIds: (data.targetProjectIds || []).join(","),
      VisitNo: data.visitNo,
...
async function saveMaintWorkflowStep(data, userName) {
  try {
    const user = await getUserRecord(userName);
    if (!data || !data.workflowId || !data.stepNo) return { status: "error", message: "\u0E02\u0E49\u0E2D\u0E21\u0E39\u0E25\u0E44\u0E21\u0E48\u0E04\u0E23\u0E1A" };
    const supabase = getSupabase();
    const { data: wfRes } = await supabase.from("MaintWorkflow").select("*").eq("WorkflowID", String(data.workflowId));
    ...
    if (!isNaN(nextStep2) && nextStep2 >= 1 && nextStep2 <= 5) {
      await supabase.from("MaintWorkflow").update({ CurrentStep: nextStep2 }).eq("WorkflowID", String(data.workflowId));
...
async function completeMaintWorkflow(data, userName) {
  try {
    const user = await getUserRecord(userName);
    ...
    const sched = data.scheduledDate || String(rowData.ScheduledDate || "") || "";
...
async function cancelMaintWorkflow(workflowId, userName) {
  try {
    const user = await getUserRecord(userName);
    const supabase = getSupabase();
    const { data: wfRes } = await supabase.from("MaintWorkflow").select("Department, Status").eq("WorkflowID", String(workflowId));
    if (!wfRes || wfRes.length === 0) return { status: "error", message: "\u0E44\u0E21\u0E48\u0E1E\u0E1A workflow" };
...
async function listDeptVault(userName) {
  try {
    const user = await getUserRecord(userName);
    ...
    const visible = items.filter((it) => userCanViewDeptVault(user, it.Department, it.IsPublic)).map(deptVaultSanitizeForClient);
    return { status: "success", items: visible };
  } catch (e) {
    return { status: "error", message: String(e && e.message ? e.message : e) };
  }
}
async function saveDeptVaultItem(data, userName) {
  try {
    const user = await getUserRecord(userName);
    ...
    const supabase = getSupabase();
    if (isEdit) {
      const { data: existing } = await supabase.from("DeptVault").select("PasswordEnc, Department, CreatedBy, CreatedAt").eq("ID", id);
      if (!existing || existing.length === 0) return { status: "error", message: "\u0E44\u0E21\u0E48\u0E1E\u0E1A\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23" };
      existingRow = existing[0];
      passwordEnc = existingRow.PasswordEnc || "";
    ...
    await supabase.from("DeptVault").upsert({
      ...
      Content: String(data.content || data.Content || "").substring(0, 5e3),
...
async function deleteDeptVaultItem(id, userName) {
  try {
    const user = await getUserRecord(userName);
    if (!user || user.role === "guest") return { status: "error", message: "\u0E01\u0E23\u0E38\u0E13\u0E32\u0E40\u0E02\u0E49\u0E32\u0E2A\u0E39\u0E48\u0E23\u0E30\u0E1A\u0E1A" };
...
async function revealDeptVaultPassword(id, userName) {
  try {
    const user = await getUserRecord(userName);
    if (!user || user.role === "guest") return { status: "error", message: "\u0E01\u0E23\u0E38\u0E13\u0E32\u0E40\u0E02\u0E49\u0E32\u0E2A\u0E39\u0E48\u0E23\u0E30\u0E1A\u0E1A" };
    if (!id) return { status: "error", message: "\u0E44\u0E21\u0E48\u0E1E\u0E1A\u0E23\u0E32\u0E22\u0E01\u0E32\u0E23" };
...
async function getLicenseTypeById(typeId) {
...
async function saveLicenseType(data, userName) {
  try {
    await initSheets();
    const user = await getUserRecord(userName);
...
async function deleteLicenseType(id, userName) {
  try {
    await initSheets();
    const user = await getUserRecord(userName);
    if (!user || normalizeUserRole(user.role) !== "admin") return { status: "error", message: "\u0E44\u0E21\u0E48\u0E21\u0E35\u0E2A\u0E34\u0E17\u0E18\u0E34\u0E4C" };
...
async function saveProjectLicense(data, userName) {
  try {
    await initSheets();
    const user = await getUserRecord(userName);
    ...
    if (alertOverride === "" || alertOverride == null) alertOverride = "";
      ...
      if (isNaN(alertOverride) || alertOverride < 1) {
...
async function deleteProjectLicense(id, userName) {
  try {
    await initSheets();
    const user = await getUserRecord(userName);
    if (!user || normalizeUserRole(user.role) === "guest") {
...
async function uploadFileToDrive(base64Data, fileName, deptName, explicitMimeType) {
  try {
    ...
    let contentType = explicitMimeType || "application/octet-stream";
...
async function listProjectFiles(_query, userName) {
  try {
    await initSheets();
    const user = await getUserRecord(userName);
    if (!user || normalizeUserRole(user.role) === "guest") {
...
function deleteProjectFile(_fileId, _userName) {
  return { status: "error", message: LOCALHOST_ONLY_MSG };
}
function renameProjectFile(_fileId, _newName, _userName) {
  return { status: "error", message: LOCALHOST_ONLY_MSG };
}
function moveProjectFile(_data, _userName) {
  return { status: "error", message: LOCALHOST_ONLY_MSG };
}
function createCustomFolder(_projectId, _name, _userName) {
  return { status: "error", message: LOCALHOST_ONLY_MSG };
}
async function listCustomFolders(_projectId, userName) {
  try {
    await initSheets();
    const user = await getUserRecord(userName);
    if (!user || normalizeUserRole(user.role) === "guest") {
...
function deleteCustomFolder(_folderId, _userName) {
  return { status: "error", message: LOCALHOST_ONLY_MSG };
}

// supabase/functions/_shared/dispatch.ts
async function dispatchApiCall(name, args) {
  const a = args || [];
  switch (name) {
    case "getAllData":
      return getAllData(a[0]);
    case "authenticateUser":
      return authenticateUser(a[0], a[1]);
    case "restoreSession":
      return restoreSession(a[0], a[1]);
    case "invalidateUserSession":
      return invalidateUserSession(a[0]);
    case "getAdminUsersData":
      return getAdminUsersData(a[0]);
    case "adminSaveUser":
      return adminSaveUser(a[0], a[1]);
    case "adminDeleteUser":
      return adminDeleteUser(a[0], a[1]);
    case "updateSystemSettings":
      return updateSystemSettings(a[0], a[1], a[2], a[3], a[4]);
    case "updateBulkStatus":
      return updateBulkStatus(a[0], a[1]);
    case "updateCycleOffset":
      return updateCycleOffset(a[0], a[1], a[2]);
    case "saveProject":
      return saveProject(a[0]);
    case "saveProjectBulk":
      return saveProjectBulk(a[0]);
    case "updateFullProject":
      return updateFullProject(a[0], a[1]);
    case "deleteProject":
      return deleteProject(a[0], a[1]);
    case "updateProjectsPhaseBulk":
      return updateProjectsPhaseBulk(a[0], a[1], a[2], a[3]);
    case "updateMaintenanceVisit":
      return updateMaintenanceVisit(a[0]);
    case "deleteMaintenanceVisit":
      return deleteMaintenanceVisit(a[0], a[1], a[2], a[3]);
    case "startMaintWorkflow":
      return startMaintWorkflow(a[0], a[1]);
    case "saveMaintWorkflowStep":
      return saveMaintWorkflowStep(a[0], a[1]);
    case "completeMaintWorkflow":
      return completeMaintWorkflow(a[0], a[1]);
    case "cancelMaintWorkflow":
      return cancelMaintWorkflow(a[0], a[1]);
    case "listDeptVault":
      return listDeptVault(a[0]);
    case "saveDeptVaultItem":
      return saveDeptVaultItem(a[0], a[1]);
    case "deleteDeptVaultItem":
      return deleteDeptVaultItem(a[0], a[1]);
    case "revealDeptVaultPassword":
      return revealDeptVaultPassword(a[0], a[1]);
    case "saveLicenseType":
      return saveLicenseType(a[0], a[1]);
    case "deleteLicenseType":
      return deleteLicenseType(a[0], a[1]);
    case "saveProjectLicense":
      return saveProjectLicense(a[0], a[1]);
    case "deleteProjectLicense":
      return deleteProjectLicense(a[0], a[1]);
    ...
    case "listProjectFiles":
      return listProjectFiles(a[0], a[1]);
    case "deleteProjectFile":
      return deleteProjectFile(a[0], a[1]);
    case "renameProjectFile":
      return renameProjectFile(a[0], a[1], a[2]);
    case "moveProjectFile":
      return moveProjectFile(a[0], a[1]);
    case "createCustomFolder":
      return createCustomFolder(a[0], a[1], a[2]);
    case "listCustomFolders":
      return listCustomFolders(a[0], a[1]);
    case "deleteCustomFolder":
      return deleteCustomFolder(a[0], a[1]);
    default:
...
async function handleHttpRequest(req) {
  ...
  try {
    ...
    if (!fn) return jsonResponse({ status: "error", message: "Missing fn/method" });
...
function startNodeServer(port = 8787) {
  const server = createServer(async (req, res) => {
    const url = `http://${req.headers.host}${req.url || "/"}`;
    const chunks = [];
    for await (const chunk of req) chunks.push(chunk);
    const init = { method: req.method, headers: req.headers };
    if (req.method === "POST" && chunks.length) {
      init.body = Buffer.concat(chunks);
    }
    const response = await handleHttpRequest(new Request(url, init));
    res.writeHead(response.status, Object.fromEntries(response.headers.entries()));
    res.end(Buffer.from(await response.arrayBuffer()));
  });
  server.listen(port, () => console.log(`PEA API :${port}`));
  return server;
}
var isMain = process.argv[1]?.includes("handler.mjs");
if (isMain) {
  if (!process.env.SUPABASE_URL) {
    process.env.SUPABASE_URL = "https://nnsxyuhiwgrqbszrhyiz.supabase.co";
  }
  startNodeServer(Number(process.env.PORT) || 8787);
}
export {
  handleHttpRequest,
  startNodeServer
